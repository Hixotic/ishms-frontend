...
import { Pill, AlertTriangle, FileText, X, CheckCircle, Activity } from 'lucide-react';
import { 
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[2000] p-4">
      <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-lg w-full overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300 border border-white/20">
        ...
        <div className="p-8 text-center relative bg-gradient-to-b from-slate-50 to-white border-b border-slate-100">
          ...
          <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center mx-auto mb-6 transition-all duration-700 shadow-inner ${
            loading ? 'bg-blue-50 text-blue-600 animate-pulse' : 
            hasRisks ? 'bg-red-50 text-red-600' : 
            isSafe ? 'bg-green-50 text-green-600' : 
            'bg-slate-100 text-slate-400'
          }`}>
            {loading ? <Activity size={40} /> : <Pill size={40} />}
          </div>
          
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Drug Interaction Check</h2>
          <p className="text-slate-500 text-sm mt-2 font-medium">Pharmacological safety analysis engine</p>
        </div>
            ...
            <div className="text-center py-6">
              <div className="bg-slate-50 rounded-3xl p-6 mb-6 border border-dashed border-slate-200">
                <p className="text-slate-500 text-sm leading-relaxed">
                  Analyze current prescriptions against the patient's clinical history to identify potential contraindications or high-risk combinations.
                </p>
              </div>
              <button 
                onClick={runCheck} 
          ...
          {loading && (
            <div className="py-16 text-center space-y-6">
              ...
              <div className="space-y-2">
                <p className="text-slate-900 font-black text-xs uppercase tracking-widest">Cross-referencing database</p>
                <p className="text-slate-400 text-[10px] font-medium uppercase tracking-tighter">Analyzing molecular compatibility...</p>
              </div>
            </div>
          )}
          ...
          {result && (
            <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Findings</span>
                <div className="h-px flex-1 bg-slate-100"></div>
              </div>
              ...
              {hasRisks ? (
                <div className="space-y-3">
                  {result.map((item, idx) => (
                    <div key={idx} className="group p-5 rounded-3xl bg-red-50/50 border border-red-100 hover:bg-red-50 hover:border-red-200 transition-all duration-300">
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          {item.risk_summary ? (
                            ...
                            <p className="text-[10px] text-red-400 italic font-medium">
                              Immediate clinical review recommended for this combination.
                            </p>
                          )}
                        </div>
                      </div>
                ...
                <div className="py-8 px-6 rounded-[2rem] bg-green-50/50 border border-green-100 text-center animate-in zoom-in duration-500">
                  ...
                  <p className="text-green-900 font-black text-base">Clearance Confirmed</p>
                  <p className="text-green-600/70 text-xs font-bold mt-2 leading-relaxed">
                    No dangerous interactions detected. The current medication regimen is clinically compatible.
                  </p>
                </div>
              )}
            </div>
