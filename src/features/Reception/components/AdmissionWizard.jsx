...
const AdmissionWizard = ({ onComplete }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(
        "Failed to complete admission. Please verify details and try again.",
      );
    } finally {
      setLoading(false);
    }
        ...
        return (
          <React.Fragment key={s.id}>
            <div className="flex flex-col items-center relative z-10">
              ...
              <div
                className={`text-xs font-black uppercase tracking-widest mt-3 whitespace-nowrap ${isActive ? "text-blue-600" : "text-slate-400"}`}
              >
                {s.title}
              </div>
            </div>
            {index < steps.length - 1 && (
              <div className="flex-1 h-px mx-3 bg-slate-100 relative">
                <div
                  className={`absolute inset-0 bg-blue-600 transition-transform duration-700 origin-left ${
  ...
  const renderStep1 = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="space-y-6">
        <div>
          <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2.5 block ml-1">
            Full Legal Name
          </label>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div>
          ...
          <div className="relative">
            ...
            >
              <option value="">
                {formData.departmentId ? "Select Bed" : "Choose Ward First"}
              </option>
              {availableBeds.map((bed) => (
                <option key={bed.bedId || bed.id} value={bed.bedId || bed.id}>
                  Bed {String(bed.bedId || bed.id).padStart(3, "0")} (
                  {bed.roomNumber || "Ward"})
                </option>
              ))}
            </select>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="bg-blue-50/50 border border-blue-100 rounded-3xl p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h4 className="text-base font-black text-slate-900 uppercase tracking-widest">
            Review Admission
          </h4>
        </div>
        <div className="space-y-4">
          <div className="flex justify-between items-start py-3 border-b border-blue-100/50 gap-4">
            ...
            <span className="text-sm font-bold text-slate-900 text-right break-words">
              {formData.fullName}
            </span>
          </div>
          <div className="flex justify-between items-start py-3 border-b border-blue-100/50 gap-4">
          ...
          <div className="flex justify-between items-start py-3 gap-4">
            ...
            <span className="text-sm font-bold text-slate-900 text-right">
              #{String(formData.bedId).padStart(3, "0")}
            </span>
          </div>
        </div>
      </div>
  ...
  const renderCompleted = () => (
    <div className="flex flex-col items-center justify-center py-16 animate-in zoom-in duration-500">
      ...
      <p className="text-slate-500 text-sm font-bold text-center max-w-xs mb-10 uppercase tracking-widest">
