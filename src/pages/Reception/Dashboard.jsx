...
import { HandleDepartmentByBedId, formatPatientId, formatBedId } from '../../api/Handler';
...
const KpiCard = ({ label, value, sub, icon: Icon, color }) => (
  <div className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm hover:shadow-md transition-all duration-300">
    <div className="flex items-center gap-3">
      <div className="rounded-2xl p-2.5 flex-shrink-0" style={{ background: `${color}15` }}>
    ...
    <div className="mt-3 text-[10px] font-bold text-slate-400 tracking-tight border-t border-slate-50 pt-2">{sub}</div>
  </div>
);
  ...
  >
    <div className="relative">
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors duration-300 ${
        ...
        patient.status === 'Unstable' ? 'bg-amber-50 text-amber-500' : 'bg-emerald-50 text-emerald-500'
      }`}>
        <User size={24} />
      </div>
      <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${
        patient.status === 'Critical' ? 'bg-red-500' :
        patient.status === 'Unstable' ? 'bg-amber-500' : 'bg-emerald-500'
      }`} />
    </div>
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="mt-1 flex flex-wrap items-center gap-2">
        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{formatPatientId(patient.id)}</span>
        <span className="w-1 h-1 rounded-full bg-slate-200" />
        <span className="text-[9px] font-black text-slate-900 uppercase tracking-widest">Bed {formatBedId(patient.bed || patient.bedId || 'TBD')}</span>
        <span className="w-1 h-1 rounded-full bg-slate-200" />
        <span className="text-[9px] font-black text-blue-600 uppercase tracking-widest">{HandleDepartmentByBedId(patient.bedId) || 'General Ward'}</span>
      </div>
    </div>
    ...
    <div className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${
      ...
      patient.status === 'Unstable' ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'
    }`}>
      {patient.status}
    </div>
...
const DischargeRow = ({ patient, onDischarge, onViewDetails }) => (
  <div className="group flex items-center gap-4 p-4 rounded-[1.5rem] border border-slate-100 mb-3 bg-white hover:border-emerald-200 transition-all duration-300">
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="mt-1 flex flex-wrap items-center gap-2">
        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Bed {formatBedId(patient.bed || patient.bedId || 'TBD')}</span>
        <span className="w-1 h-1 rounded-full bg-slate-200" />
...
const QuickBtn = ({ icon: Icon, label, onClick, color }) => (
  <button
    ...
    onClick={onClick}
  >
    <div className="w-9 h-9 rounded-xl flex items-center justify-center transition-colors duration-300 flex-shrink-0" style={{ background: `${color}15`, color }}>
      <Icon size={18} />
    </div>
    <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest group-hover:text-blue-600">{label}</span>
  </button>
);

// ─── MAIN OVERVIEW TAB ───────────────────────────────────────────────

const DashboardOverview = ({ patients, alerts, handleAdmit, handleDischarge, handleView, handleQuickAction }) => {
  ...
  const filteredPatients = patients.filter((p) => {
    ...
    return `${p.fullName || p.name || ''}`.toLowerCase().includes(searchTerm.toLowerCase());
  });

  const admittedPatients = filteredPatients.filter(p => p.bedId);
  const todaysDischarges = filteredPatients.filter((p) => p.status === 'Discharged').length;
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <KpiCard label="Total Admitted" value={admittedPatients.length} sub="Currently in ward" icon={Users} color="#3b82f6" />
        <KpiCard label="Today's Discharges" value={todaysDischarges} sub="Medically cleared" icon={DoorOpen} color="#10b981" />
        <KpiCard label="Active Alerts" value={openAlerts} sub="Requires attention" icon={AlertCircle} color="#f59e0b" />
      </div>
          ...
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              <div className="space-y-1">
                <h2 className="text-xl font-black text-slate-900 uppercase tracking-wider">Admitted Patients</h2>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Real-time ward status</p>
              </div>
            ...
            <div className="space-y-1">
              {admittedPatients.length > 0 ? (
                admittedPatients.slice(-5).reverse().map((patient) => (
                  <AdmittedPatientRow
                    key={patient.id}
                    patient={patient}
                ...
                <div className="py-10 text-center">
                  ...
                  <p className="text-slate-400 text-xs font-black uppercase tracking-widest">No patients admitted</p>
                </div>
              )}
            </div>
            
            <button className="w-full mt-6 py-4 rounded-2xl border border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:bg-slate-50 hover:text-blue-600 transition-all duration-300 flex items-center justify-center gap-2"
        ...
        <div className="xl:col-span-5 space-y-8">
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <QuickBtn icon={UserPlus} label="New Admission" color="#3b82f6" onClick={() => handleQuickAction('admission')} />
              <QuickBtn icon={BedDouble} label="Bed Map" color="#3b82f6" onClick={() => handleQuickAction('beds')} />
              <QuickBtn icon={AlertCircle} label="Active Alerts" color="#3b82f6" onClick={() => handleQuickAction('alerts')} />
              <QuickBtn icon={ChartBar} label="Daily Reports" color="#3b82f6" onClick={() => handleQuickAction('reports')} />
              <QuickBtn icon={DoorOpen} label="Discharges" color="#3b82f6" onClick={() => handleQuickAction('discharge')} />
              <QuickBtn icon={ShieldCheck} label="Activity Logs" color="#3b82f6" onClick={() => handleQuickAction('logs')} />
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <div className="space-y-1">
                <h2 className="text-lg font-black text-slate-900 uppercase tracking-wider">Discharge Queue</h2>
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Medically Cleared</p>
              </div>
            ...
            <div className="space-y-1">
              {dischargePatients.length > 0 ? (
                ...
                <div className="py-8 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                  ...
                  <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest">Queue is clear</p>
                </div>
              )}
            </div>
          </div>
...
export default function ReceptionDashboard() {
  ...
  const currentTab = ['reception', 'reception-tasks'].includes(currentPath) ? 'dashboard' : currentPath;
  ...
  const renderTabContent = () => {
    switch (currentTab) {
      ...
      case 'admission': return <AdmissionWizard />;
      case 'beds': return <BedManagement />;
      case 'discharge': return <DischargeMonitor />;
      case 'alerts': return <AlertsPanel alerts={alerts} loading={<RefreshCw size={13} />} />;
      case 'Analytics': return <ExecutivePage />;
      default: return <DashboardOverview patients={patients} alerts={alerts} handleQuickAction={handleQuickAction} />;
    }
  };
