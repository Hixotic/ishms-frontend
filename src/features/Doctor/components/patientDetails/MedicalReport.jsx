...
import { X, AlertTriangle, FileText, Save, Clipboard, Activity, ChevronRight} from 'lucide-react';
...
const MedicalReportModal = ({ patient, onClose, onSave, existingReport }) => {
  ...
  const handleSubmit = async () => {
    if (!data.diagnosis || !data.treatmentPlan) return alert('Please fill all required fields');
    setLoading(true);
    try {
        ...
        <div className="bg-slate-900 text-white p-6 relative overflow-hidden">
          <div className="relative flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-black tracking-tight leading-tight">Medical Report</h2>
                <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest mt-0.5">
                  Patient: {patient.fullName} • ID: {String(patient.id || '').substring(0, 6)}
                </p>
              </div>
            </div>
            <button 
        ...
        {!canWrite ? (
          ...
          <div className="p-12 text-center space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-black text-slate-900">Action Restricted</h3>
              <p className="text-slate-500 text-sm font-medium leading-relaxed max-w-xs mx-auto">
                Reports can only be drafted when the patient flow status is <span className="text-amber-600 font-bold">"Waiting Doctor"</span>.
              </p>
            </div>
            <button 
              onClick={onClose} 
