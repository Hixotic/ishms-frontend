...
import PatientDetailsModal from '../Reception/PatientDetailsModal';
import { useData } from './IContext';
import { formatPatientId } from '../../api/Handler';
...
export default function MainLayout() {
  ...
  const navItems = useMemo(() => {
    const common = [
      { to: '/', label: 'Dashboard', icon: <LayoutDashboard size={18} />, section: 'Overview' }
    ];

    const receptionItems = [
      { to: '/admission', label: 'New Admission', icon: <UserPlus size={18} />, section: 'Operations' },
      { to: '/beds', label: 'Bed Management', icon: <LayoutGrid size={18} />, section: 'Operations' },
      { to: '/discharge', label: 'Discharge Monitor', icon: <DoorOpen size={18} />, section: 'Operations' },
      { to: '/DoctorAlertsPage', label: 'Alerts', icon: <AlertCircle size={18} />, section: 'Information' },
      { to: '/executive', label: 'Analytics', icon: <ChartBar size={18} />, section: 'Information' }
    ];

    const doctorItems = [
      { to: '/TasksPage', label: 'Tasks Panel', icon: <ClipboardList size={18} />, section: 'Operations' },
      { to: '/DoctorAlertsPage', label: 'Alerts Panel', icon: <AlertCircle size={18} />, section: 'Operations' },
      { to: '/executive', label: 'Analytics', icon: <BarChart3 size={18} />, section: 'Information' }
    ];
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-gray-50 font-sans text-slate-900">
      ...
      {isMobile && sidebarOpen && (
        <div className="fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-sm transition-opacity duration-300" onClick={toggleSidebar} />
      )}
      ...
      `}>
        {/* LOGO */}
        <div className={`flex items-center justify-between border-b border-slate-100 h-20 ${sidebarOpen ? 'px-6' : 'px-4'}`}>
          {sidebarOpen ? (
            <div className="flex flex-col">
              <span className="text-2xl font-black text-blue-600 tracking-tight leading-none">iSHMS</span>
              <span className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-[0.2em]">{role} Portal</span>
            </div>
          ) : (
          ...
          {!isMobile && (
            <button onClick={toggleSidebar} className="p-2 rounded-xl hover:bg-slate-50 text-slate-400 hover:text-blue-600 transition-all">
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          <div className={`flex items-center gap-3 p-2 rounded-xl transition-all ${sidebarOpen ? 'bg-white shadow-sm border border-slate-100' : 'justify-center'}`}>
            ...
            {sidebarOpen && (
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-slate-800 truncate">{auth.user?.fullName || 'User'}</p>
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 md:px-10 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            {isMobile && (
              <button onClick={toggleSidebar} className="p-2 rounded-xl bg-slate-100 text-slate-600">
                <Menu size={20} />
              </button>
            )}
              ...
              <div className="absolute top-full left-0 right-0 mt-3 bg-white border border-slate-200 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                {filteredPatients.length > 0 ? (
                    ...
                    {filteredPatients.map(p => (
                      ...
                      >
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${
                          p.status === 'Critical' ? 'bg-red-500' : p.status === 'Unstable' ? 'bg-amber-500' : 'bg-emerald-500'
                        }`}>
                          {p.name?.charAt(0) || 'P'}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-slate-800">{p.name || p.fullName}</p>
                          <p className="text-xs font-medium text-slate-500">{formatPatientId(p.id)} • {p.bedId || p.bed || 'No Bed'}</p>
                        </div>
                        <ChevronRight size={16} className="text-slate-300 group-hover/item:text-blue-600 transition-transform group-hover/item:translate-x-1" />
                      </button>
                    ))}
                  </div>
                ) : (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-xs text-slate-500 mt-1">Try a different name or patient Id</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <button
                ...
                className={`notif-btn w-11 h-11 rounded-xl border flex items-center justify-center transition-all ${
                  notifOpen ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-slate-200 text-slate-500 hover:border-blue-300 hover:text-blue-600'
                }`}
              >
                <Bell size={20} />
                {alerts.length > 0 && (
                  ...
                  <div className="max-h-[400px] overflow-y-auto p-2 space-y-1">
                    {alertsLoading ? (
                      ...
                      <div className="p-10 text-center">
                        ...
                        <p className="text-xs text-slate-500 mt-1">No new alerts to show</p>
                      </div>
                    ) : (
                      alerts.slice(0, 5).map((alert) => (
                        <button
                          ...
                          onClick={() => {
                            setNotifOpen(false);
                            navigate(isDoctor ? '/DoctorAlertsPage' : '/alerts');
                          }}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-slate-800 line-clamp-1">{alert.title || alert.type || 'System Alert'}</p>
            ...
            <div className="hidden sm:flex flex-col items-end mr-2">
              <span className="text-xs font-bold text-slate-800">{auth.user?.fullName?.split(' ')[0] || 'User'}</span>
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{role}</span>
            </div>
          </div>
        </header>
...
function SidebarLink({ to, label, icon, collapsed, isActive }) {
  return (
    ...
    >
      <span className={`transition-transform group-hover:scale-110 ${isActive ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500'}`}>
        {icon}
      </span>
