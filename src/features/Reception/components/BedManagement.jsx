...
function useBedData() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const [deptsResponse, availableResponse, occupiedResponse] =
        await Promise.all([
          getDepartments(),
          getAvailableBeds(),
          getOccupiedBeds(),
    ...
    return (
      <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-36 rounded-xl bg-white text-slate-800 text-xs px-3 py-2 shadow-2xl border border-slate-100 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-150 whitespace-nowrap">
        ...
        <div className="text-slate-500">
          Bed {String(bed.id).padStart(3, "0")}
        </div>
  ...
  return (
    <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 rounded-xl bg-white text-slate-900 text-xs px-3 py-2.5 shadow-2xl border border-slate-100 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-150">
      ...
      <div className="flex items-center gap-1.5 mb-1">
        <span
          className={`inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${status.bg} ${status.text}`}
        >
          <span className={`w-1.5 h-1.5 rounded-full ${status.dot}`} />
          {patient.flowStatus}
        </span>
        <span
          className={`rounded-full px-1.5 py-0.5 text-[10px] font-black text-white ${news.badge}`}
        >
          NEWS {patient.newsScore}
        </span>
      </div>
      <div className="text-slate-500 flex items-center gap-1">
        <User size={9} /> Age {patient.age}
      </div>
...
function BedCell({
  ...
  if (view === "list") {
    ...
    const status = patient
      ? STATUS_STYLE[patient.flowStatus] || STATUS_STYLE.Stable
      : null;
    return (
      <div
        onClick={handleClick}
      ...
      >
        <div
          className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isOccupied ? "bg-blue-100" : isAvailable ? "bg-emerald-100" : "bg-slate-100"}`}
        >
          <BedDouble
            size={14}
            className={
          ...
          <>
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-slate-400">
                Age {patient.age} · {timeAgo(patient.admittedAt)}
              </div>
            </div>
            <span
              className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${status.bg} ${status.text}`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${status.dot}`} />
              {patient.flowStatus}
            </span>
            <span
              className={`rounded-full px-2 py-0.5 text-[10px] font-black text-white ${news.badge}`}
            >
              NEWS {patient.newsScore}
            </span>
          </>
        ) : isAvailable ? (
          <div className="flex-1 text-xs text-emerald-600 font-medium">
            Available
          </div>
        ) : (
            ...
            isOccupied
              ? `cursor-pointer ${
                  patient.newsScore >= 7
                    ...
                    : patient.flowStatus === "UnderObservation" ||
                        patient.flowStatus === "WaitingDoctor"
      ...
      >
        {isOccupied ? (
          <>
            ...
            <div className="text-[10px] font-black text-slate-600 leading-none">
              {String(bedId).padStart(3, "0")}
            </div>
            {patient.newsScore >= 4 && (
              <div
                className={`absolute top-1 right-1 w-2 h-2 rounded-full ${patient.newsScore >= 7 ? "bg-red-500 animate-pulse" : "bg-amber-400"}`}
              />
            )}
          </>
        ) : isAvailable ? (
          ...
          <>
            ...
            <div className="text-[10px] font-black text-slate-300 leading-none">
              {String(bedId).padStart(3, "0")}
            </div>
          </>
        )}
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
          {[
            ...
            <div
              key={label}
              className={`rounded-2xl border ${border} ${bg} p-3 sm:p-4 flex items-center gap-3 sm:gap-4`}
            >
        ...
        <div className="mb-4 flex items-center gap-2 sm:gap-4 flex-wrap text-xs">
          ...
          {[
            ...
            { color: "bg-blue-200 border border-blue-300", label: "Stable" },
            {
              ...
              label: "Under Observation / Waiting",
            },
          ...
          <div className="flex items-center gap-1.5 ml-auto text-blue-600 font-medium">
            ...
            <span className="hidden sm:inline">
              Click occupied bed to transfer
            </span>
